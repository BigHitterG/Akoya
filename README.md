...
## FedEx fallback shipping table

`POST /api/get-fedex-rate` now supports a final fallback table when live FedEx quotes fail or FedEx config is unavailable.

- Default fallback table is seeded in `api/lib/fallback-shipping-rates.js`.
- Override at runtime with `FEDEX_FALLBACK_RATE_TABLE_JSON` (JSON object keyed by quantity, value in USD).
- Response includes `fallbackUsed: true` and `fallbackReason` whenever fallback pricing is applied.

Example:

```bash
FEDEX_FALLBACK_RATE_TABLE_JSON={"1":20.71,"2":25.07,"3":32.64,"4":40.52,"5":56.52,"6":56.52,"7":70.53,"8":70.53,"9":133.61,"10":133.61}
```
...
```bash
FALLBACK_RATE_REFRESH_TOKEN=long_random_secret
FALLBACK_RATE_REPORT_TO=ops@yourdomain.com
FALLBACK_RATE_REPORT_FROM=orders@yourdomain.com
```
...
Operationally, a **weekly refresh** is a good default and keeps noise low, with an additional manual refresh around known annual carrier rate changes (for FedEx this is commonly early January).
