...
module.exports = async function handler(req, res) {
  ...
  const lines = [
    'New Akoya Pediatric Character Mask launch interest',
    '',
    `Name: ${name}`,
    `Email: ${email}`,
    `Organization: ${organization}`,
    `Estimated quantity: ${quantity}`,
    `Purchase timing: ${timing || 'Not provided'}`,
  ...
  try {
    const providerResponse = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        ...
        to: [notifyTo],
        reply_to: email,
        subject: `Pediatric launch interest — ${organization}`,
        text: lines.join('\n')
      })
    });
    ...
    await sendCustomerEmail({
      toEmail: email,
      subject: 'Akoya Pediatric Character Mask launch interest received',
      textLines: [
        `Hello ${name},`,
        '',
        'Thank you for registering your interest in the Akoya Pediatric Character Mask.',
