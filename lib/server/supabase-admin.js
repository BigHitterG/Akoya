const DEFAULT_SHIPPING_LABELS_BUCKET = 'shipping_labels';
...
async function createSiteVisitRecord(record) {
  const { supabaseUrl } = getSupabaseConfig();
  const response = await fetch(`${supabaseUrl}/rest/v1/${SITE_VISITS_TABLE}`, {
    method: 'POST',
    headers: {
      ...getAuthHeaders('application/json'),
  ...
  if (!response.ok) {
    ...
    const message = details ? `: ${details.slice(0, 500)}` : '';
    throw new Error(`Supabase site visit insert failed (${response.status})${message}`);
  }
}
